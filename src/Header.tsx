...
let Header = observer(({setOrder, block, order}: HeaderProps) => {
    ...
    return (
        <React.Fragment>
            {(!hidden)?
            <>
            <div className='w-full bg-[#023E8A] flex flex-row py-7 pl-[10%] pr-[10%]'>
                ...
                <button onClick={(e)=>{e.stopPropagation();setShowCart((cart)=>!cart)}} className='border-[2px] relative border-white px-2 mr-16 ml-auto rounded-full flex items-center justify-center'>{(store.items.length > 0)?<span className='absolute translate-x-5 -translate-y-5 bg-red-500 rounded-full w-[15px] h-[15px]'/>:<></>}<img className='h-[32px] w-[32px]' src={cart}/></button>
            </div>
            </>
            :
            <>
            <div className='w-full mt-24'></div>
            <div className='w-full z-30 fixed bg-[#023E8A] flex flex-row py-2 pl-[10%] pr-[10%]'>
                ...
                <div className='flex-row flex space-x-6'>
                    ...
                    <HButton choosen={block===Block.Food} location='food' hidden={true}>Вкусняшки</HButton>
            ...
            <button onClick={(e)=>{e.stopPropagation();setShowCart((cart)=>!cart)}} className='border-[2px] relative border-white mr-16 ml-auto rounded-full'>{(store.items.length > 0)?<span className='absolute translate-x-2 -translate-y-1 bg-red-500 rounded-full w-[10px] h-[10px]'/>:<></>}<img className='h-[28px] w-[28px] p-1' src={cart}/></button>
            </div>
            </>
            }
        {(showCart && !order)?<Cart setOrder={setOrder} hidden={hidden}/>:<></>}
        </React.Fragment>
    );
})
